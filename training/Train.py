...
train.py
--------
Pipeline d'entraînement unifié pour MLP, LSTM/GRU et CNN.
Gère : early stopping, scheduler, logging, sauvegarde des meilleurs modèles.
"""
...
def make_dataloader(
    ...
    """
    Crée un DataLoader à partir de tableaux numpy.
    Gère les formats différents selon le type de modèle.
    """
...
class Trainer:
    """
    Entraîne n'importe quel modèle PyTorch avec :
      - Cross-entropy loss
      - Adam optimizer
    ...
    def __init__(
        self,
        model:       nn.Module,
        model_type:  str,                     # "mlp" | "lstm" | "gru" | "cnn"
        ...
        for epoch in range(1, epochs + 1):
            ...
            if val_loss < best_val_loss:
                ...
                if patience_counter >= patience:
                    if verbose:
                        print(f"\nEarly stopping à l'époque {epoch} (meilleur : {best_epoch})")
                    break
    ...
    def _run_epoch(
        ...
        """Un passage sur le loader (train ou eval)."""
    ...
    def _forward(self, x: torch.Tensor) -> torch.Tensor:
        """Gère le cas AttentionLSTM qui retourne (logits, weights)."""
        out = self.model(x)
    ...
    def predict(self, X: np.ndarray, batch_size: int = 256) -> np.ndarray:
        """Retourne les probabilités [P(down), P(up)] pour chaque sample."""
...
# ---------------------------------------------------------------------------
# Point d'entrée — entraînement rapide de démonstration
# ---------------------------------------------------------------------------
